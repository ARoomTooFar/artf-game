...
public class ARTFRoomManagerTests : MasterTest{
	...
	public void addRoomTest(){
		...
		rm.room(new Vector3(0, 0, 0), new Vector3(0, 0, 0));
		Assert.AreEqual(1, rm.RoomList.Count);
		rm.room(new ARTFRoom(new Vector3(1, 0, 1), new Vector3(1, 0, 1)));
		Assert.AreEqual(2, rm.RoomList.Count);
		rm.room(new Vector3(0, 0, 0), new Vector3(1, 0, 1));
		Assert.AreEqual(2, rm.RoomList.Count);
	}
	...
	public void doAnyRoomsIntersectTest(){
		...
		Assert.IsFalse(rm.isMoveValid(room, new Vector3(1, 0, 1)));

		ARTFRoom r1 = new ARTFRoom(new Vector3(0, 0, 0), new Vector3(0, 0, 0));
		Assert.IsTrue(rm.isMoveValid(r1));

		ARTFRoom r2 = new ARTFRoom(new Vector3(-1, 0, -1), new Vector3(-1, 0, -1));
		Assert.IsFalse(rm.isMoveValid(r2));
		Assert.IsTrue(rm.isMoveValid(r2, new Vector3(1, 0, 1)));
	...
	public void moveRoomTest(){
		...
		Assert.IsFalse(rm.move(new Vector3(0, 0, 0), new Vector3(1, 0, 1)));
		Assert.AreEqual(new Vector3(0, 0, 0), r1.LLCorner);
		Assert.IsTrue(rm.move(new Vector3(3, 0, 3), new Vector3(1, 0, 1)));
	...
	public void resizeRoomTest(){
		...
		Assert.IsFalse(rm.resize(new Vector3(2, 0, 2), new Vector3(3, 0, 3)));
		Assert.AreEqual(new Vector3(2, 0, 2), r1.URCorner);

		Assert.IsFalse(rm.resize(new Vector3(5, 0, 5), new Vector3(4, 0, 4)));
		Assert.AreEqual(new Vector3(6, 0, 6), r2.URCorner);

		Assert.IsTrue(rm.resize(new Vector3(6, 0, 6), new Vector3(7, 0, 7)));
