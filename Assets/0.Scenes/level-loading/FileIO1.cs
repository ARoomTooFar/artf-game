...
public class FileIO1 : MonoBehaviour
{
	...
	private string dummyLvlId = "5713573250596864";
	#endif
	...
	public IEnumerator dlLvl (WWW www)
	{
		...
		if (serv.dataCheck (lvlData)) {
			...
			BullyTrunk[] bt = FindObjectsOfType (typeof(BullyTrunk)) as BullyTrunk[];
			foreach (BullyTrunk io in bt) {
				GameObject ob = io.gameObject;
				ob.GetComponent<BullyTrunk> ().enabled = true;

				GameObject ar = ob.transform.Find("AggroRange").gameObject;
				ar.SetActive(true);
			}
			
			CackleBranch[] cb = FindObjectsOfType (typeof(CackleBranch)) as CackleBranch[];
			foreach (CackleBranch i in cb) {
				GameObject ob = i.gameObject;
				ob.GetComponent<CackleBranch> ().enabled = true;

				GameObject ar = ob.transform.Find("AggroRange").gameObject;
				ar.SetActive(true);
			}

			TestSwordEnemy[] tse = FindObjectsOfType (typeof(TestSwordEnemy)) as TestSwordEnemy[];
			foreach (TestSwordEnemy i in tse) {
				GameObject ob = i.gameObject;
				ob.GetComponent<TestSwordEnemy> ().enabled = true;
